...
except ImportError:  # The original exact implementation remains available.
...
def _levenshtein_ops_python(ref: list[str], hyp: list[str]) -> dict[str, int]:
    n = len(ref)
    m = len(hyp)
...
def levenshtein_ops(ref: list[str], hyp: list[str]) -> dict[str, int]:
    """Return exact unit-cost Levenshtein operation counts.

    RapidFuzz avoids the quadratic Python object matrix used by the reference
    fallback.  This is essential for dataset-level character sequences while
    preserving the same Levenshtein distance and error-rate definition.
    """
    if _RapidFuzzLevenshtein is None:
        return _levenshtein_ops_python(ref, hyp)
    substitutions = 0
    deletions = 0
    insertions = 0
    for operation in _RapidFuzzLevenshtein.editops(ref, hyp):
        tag = operation.tag
        if tag == "replace":
            substitutions += 1
        elif tag == "delete":
            ...
            insertions += 1
        else:  # Defensive guard for future RapidFuzz operation tags.
            raise ValueError(f"Unsupported Levenshtein edit operation: {tag}")
    return {
        "substitutions": substitutions,
        "deletions": deletions,
        "insertions": insertions,
        "matches": len(ref) - substitutions - deletions,
    }
